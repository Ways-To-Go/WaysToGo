...
export class EnteteVoyage extends Component {
	...
	getStageNumber(etapes) { return etapes.length; }
	...
	render() {
		return (
			<div ref={this.state.param.refHead} id="head">
				...
				<h5>{this.state.param.startdate ? '' : this.getMonthsDate1(this.state.param.stages)}</h5>
				...
				{this.state.param.edit ?
					<div>
						<label for="files2" style={{ border: "2px solid black" }}>Change cover image</label>
						<input id="files2" style={{ visibility: "hidden" }} ref={this.state.param.refHead} type="file" accept="image/*" onChange={this.state.param.changeCover}/>
					</div>
					 : ""}
			</div>
		);
			...
			<div>
				<div class="etape">
					...
					{this.props.etape.photos.map((photo, i) => (
						<div>
						<img key={i} src={photo.path} alt={photo.title} height="150" width="200"></img>
		...
		xhttp.onreadystatechange = function () {
			if (this.readyState === 4 && this.status === 200) {
				...
				parentVoyage.setState({
					toRender:
						<div>
							<NavigationBar token={parentVoyage.props.token} />
							<div id="carteSide">
